...
def post_playlist(access_token, spotify_user_id, title):
    for attempt in range(0, SPOTIFY_API_RETRIES):
        try:
            headers = {'Authorization': f'Bearer {access_token}'}
            data = {'name': f'{title}', 'description': 'Generated by sounday'}
...
def search_for_tracks(args):
    ...
    for attempt in range(0, SPOTIFY_API_RETRIES):
        try:
            ...
            track_response = requests.get(endpoint, params=params, headers=headers, timeout=SPOTIFY_API_TIMEOUT)
